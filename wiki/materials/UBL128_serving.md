...
凡是在 §1.6 三档之间发生变化的维度，**全部** 抽出来作为配置项；代码内部对这些维度做参数化（典型上以 `sharded_tensor` 的 `shard_shape` × `rank_shape` + 角色拓扑表达，参见 [`sharded_tensor.md`](https://github.com/hw-native-sys/pypto_top_level_documents/blob/main/sharded_tensor.md)）。
