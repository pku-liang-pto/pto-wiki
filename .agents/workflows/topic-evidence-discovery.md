...
4. Use `.agents/workflows/concept-evidence-lookup.md` for important concepts, acronyms, platform components, protocols, APIs, runtime layers, or repository-specific identifiers that need stable definitions.
5. Identify the smallest relevant repository set. Use `repositories/<repository-name>/` for local checkouts when source inspection is needed.
6. Search local source and history when available:
...
7. Search GitHub evidence with `gh` when available:
...
8. Expand the evidence set from strong matches:
...
10. Identify the foundation layer required by the topic: basic terms, non-distributed execution flow, repository roles, examples, and current implementation status.
11. Inspect primary and supporting evidence deeply enough to explain the topic in human terms.
12. Write or update the smallest useful page under `wiki/topics/` with standalone synthesis, not a dump of links.
13. When the topic is advanced or distributed, cite or add the foundation pages needed to prevent future distributed-only bias.
14. When the topic is example-driven, include background concepts, beginner-to-expert progression, cross-repository comparison, optimization techniques, and TODO/design-intended missing examples.
15. When the topic uses material, GitHub, external-document, or cross-repository evidence, create or update the paired `wiki/evidence/<topic>.md` ledger.
16. Update `wiki/topics/index.md`, `wiki/evidence/index.md` when evidence pages change, `wiki/index.md`, `wiki/overview.md` when warranted, and `wiki/log.md`.
...
- important concept definitions with local explanation, source-backed status, and links to reusable concept pages when they exist
